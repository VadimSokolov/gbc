...
def xi_se(x):
    """The stationary GEV fit, the xi MLE and its numerical-Hessian SE.

    The fit is returned as well so callers can read the upper endpoint off the
    same fit the SE came from, rather than refitting and risking two answers.
    """
    ...
    return f, f["xi"], float(np.sqrt(max(cov[2, 2], 1e-8)))
...
def main():
    ...
    xis, ses, trends, endpt = {}, {}, {}, {}
    for s in stations:
        xs_, Ts_, _ = aligned(d, h, s)
        fit_s, xi_hat, se = xi_se(xs_)
        xis[s], ses[s] = xi_hat, se
        # Upper endpoint mu - sigma/xi, finite only for xi < 0.  The prose quotes
        # the extremes of this set, so every station's is ledgered.
        endpt[s] = (fit_s["mu"] - fit_s["sigma"] / xi_hat) if xi_hat < 0 else float("inf")
    ...
    for i, s in enumerate(stations):
        ...
        for v, nm in [(mle[i], "ximle"), (hs_mean[i], "xihs"),
                      (hs_lo[i], "xihs_lo"), (hs_hi[i], "xihs_hi"),
                      (kappa[i], "kappa"), (trends[s], "trend"),
                      (endpt[s], "endpoint")]:
            ledger(v, f"sp:{s}:{nm}", "{:+.3f}")

    # Two summaries the prose quotes as a range.  They were written by hand once
    # and then went stale when the horseshoe scale fix moved every kappa, so they
    # are computed here and read out of the ledger instead.
    #   shrink_pct: how far the horseshoe pulls each station's deviation from the
    #     group mean of the MLEs back toward that mean.
    #   ciratio:    the 90% horseshoe credible width over the single-station
    #     normal 90% width 2 * 1.645 * SE, so 1.0 means no pooling.
    gm = float(mle.mean())
    shrink_pct = 100.0 * (1.0 - np.abs(hs_mean - gm) / np.abs(mle - gm))
    ciratio = (hs_hi - hs_lo) / (2.0 * 1.6449 * se)
    ledger(float(shrink_pct.min()), "sp:shrink_pct_min", "{:.1f}")
    ledger(float(shrink_pct.max()), "sp:shrink_pct_max", "{:.1f}")
    ledger(float(ciratio.min()), "sp:ciratio_min")
    ledger(float(ciratio.max()), "sp:ciratio_max")
    print(f"shrink toward group mean {shrink_pct.min():.1f}-{shrink_pct.max():.1f}%"
          f"  CI width ratio {ciratio.min():.2f}-{ciratio.max():.2f}")
