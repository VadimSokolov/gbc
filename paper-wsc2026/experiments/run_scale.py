...
STAMP = time.strftime("%Y-%m-%d")   # never hardcode: the date column is provenance
N_RET = 100
MIN_YEARS = 50
...
def reset_ledger():
    ...
    Every other driver did this; run_scale.py did not, so the June and August
    runs both survived and eight scale:* identifiers carried two values with
    nothing marking which one the paper used.
    """
    if not os.path.exists(_LEDGER):
        return
    with open(_LEDGER) as fh:
        keep = [ln for ln in fh if "\trun_scale.py\t" not in ln]
...
def _sim_standardised(n_sim, target, rng, N=N_RET):
    ...
    for _ in range(n_sim):
        ...
        # Inverse-CDF, not np.clip: clipping N(0, 0.3^2) to [-0.8, 0.5] leaves a
        # 4.8% point mass at 0.5, which the training set then teaches the network.
        xi = _truncated_normal(rng, 0.0, 0.3, -0.8, 0.5)
...
def train_amortised(target, n_sim=60000, epochs=2000, seed=1):
    # The "draw" target is a fresh annual maximum, so tau indexes the tail of the
    # data distribution and the parametric head is needed to extrapolate past the
    # levels seen in training.  The "rl" target is the scalar z_N, whose tau
    # indexes posterior uncertainty about a parameter and stays concentrated.
    X, y = _sim_standardised(n_sim, target, np.random.default_rng(seed))
    return train_iqn(X, y, epochs=epochs, seed=seed, tail=(target == "draw"))
...
def main():
    ...
    with open(os.path.join(ROOT, "tab", "scale.tex"), "w") as fh:
        ...
        fh.write(f"Procedure & one network, {Nst} forward passes & "
                 f"one timed chain/station ({len(sub)} stations) \\\\\n")
        fh.write(f"Marginal cost per station & {ms_gbc:.2f}\\,ms & {1000*t_mcmc_per:.0f}\\,ms per chain "
                 f"(${marg_ratio:.0f}\\times$) \\\\\n")
        fh.write(f"One-off training & {t_train_rl:.0f}\\,s (return-level net) & n/a \\\\\n")
        fh.write(f"LOYO coverage, GBC grid interval & \\multicolumn{{2}}{{c}}{{{cov_scale:.2f} "
                 f"over {tot} station-years}} \\\\\n")
        fh.write(f"LOYO grid CRPS, GBC ($^\\circ$C) & \\multicolumn{{2}}{{c}}{{{crps_scale:.2f}}} \\\\\n")
