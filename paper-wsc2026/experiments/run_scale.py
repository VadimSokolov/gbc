...
def main():
    ...
    with open(os.path.join(ROOT, "tab", "scale.tex"), "w") as fh:
        ...
        # "N chains" conflated stations with chains: the benchmark runs four
        # chains at each of the 25 timed stations, not 25 chains in total.
        fh.write(f"Procedure & one network, {Nst} forward passes & "
                 f"{len(sub)} stations $\\times$ 4 chains \\\\\n")
